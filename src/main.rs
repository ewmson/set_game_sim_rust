...
fn choose_set(
    sets: &HashSet<(SetCard, SetCard, SetCard)>,
    _board: &HashSet<SetCard>,
) -> (SetCard, SetCard, SetCard) {
...
fn find_sets(board: &HashSet<SetCard>) -> HashSet<(SetCard, SetCard, SetCard)> {
    board
        .iter()
        .tuple_combinations()
        .filter(|(one, two, three)| is_set(&one, &two, &three))
...
fn is_set(one: &SetCard, two: &SetCard, three: &SetCard) -> bool {
    ...
    satisfy_set(&one.color, &two.color, &three.color)
        && satisfy_set(&one.shading, &two.shading, &three.shading)
        && satisfy_set(&one.shape, &two.shape, &three.shape)
        && satisfy_set(&one.number, &two.number, &three.number)
}
fn satisfy_set(
    one: &SetConstraintOption,
    two: &SetConstraintOption,
    three: &SetConstraintOption,
) -> bool {
